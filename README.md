...
接口写错，一般是 "Error: Request failed with status code 404"
...
1、直接评论： 在footer组件中处理发送评论的业务，并$emit发送事件让父组件监听完成刷新的业务

2、评论页面的回复，也就是父组件的回复处理 弹出输入框,
通常父传子是通过props，因为是点击才传递，所以建立一个点击事件，将数据先存储在data里，触发事件通过props传递；
子组件（发送评论组件）接受到数据，整理，发起请求完成回复评论的业务。

3、子组件的回复
由于每个人的评论框是一样的，所以静态页面做了组件封装，那么此层的评论回复时，属于子组件传递数据给子组件，兄弟组件传值，方法有很多，但上一步已经完成父传子，所以子——父——子的形式是比较方便的。
点击组件的回复，发起事件传递数据，父组件接收数据，又进行数据存储，所以调用同样的方法。发送评论的子组件接收完成业务处理。

4、递归组件的回复
由于服务器返回的评论嵌套是未知的，因此使用组件递归完成业务渲染，那么在对这部分的业务处理也增加了难度。同样需要事件发送和数据传递，但递归里的监听需要层层监听，层层递出，所以在递归里添加事件的监听和发出。 
