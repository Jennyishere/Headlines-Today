<template>
  <input class="myinput" type="text" @input="handleInput" @blur="handleBlur" :class="{'red':isRed,'green':!isRed}">
</template>
...
export default {
    ...
    methods: {
        ...
        handleBlur(event) {
            if(this.rules && !this.rules.test(event.target.value)) this.$toast({
                message: this.msg,
                duration: 2000
            });
